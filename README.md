...
- [⚠️ Ограничения и совместимость](#ограничения-и-совместимость)
- [🚀 Установка](#установка)
- [📄 Скрипты и описание](#support-utils)
    - [Базовая диагностика сети](#network_diagbat--базовая-диагностика-сети)
    - [Базовая диагностика ОС Windows](#pre_diagbat--базовая-диагностика-ос-windows)
        - [Анализ логов и дампов](#анализ-логов-и-дампов)
    - [Автоматизации миграции пользовательского окружения](#office_profile_exportbat--демонстрация-автоматизации-миграции-пользовательского-окружения-office)
    - [CLI-пакет автоматического резервного копирования](#autobackuptools--cli-пакет-автоматического-резервного-копирования-ключевых-пользовательских-и-системных-данных)
    - [Диагностика и оптимизация аккумулятора](#batterymasterbat--диагностика-и-оптимизация-аккумулятора)
    - [Настройка RDP-клиента и сервера](#rdp_client_setupbat-и-rdp_server_setupbat--настройка-rdp-клиента-и-сервера)
        - [Настройка сервера RDP](#rdp_server_setupbat--настройка-сервера-rdp)
        - [Диагностика и подключение клиента](#rdp_client_setupbat--диагностика-и-подключение-клиента)
    - [🧾 Сбор логов установщика и анализ через WiLogUtl](#сбор-логов-установщика-и-анализ-через-wilogutl)
        - [📊 Анализ логов установки через WiLogUtl](#анализ-логов-через-wilogutl)
...
Полуавтоматический инструмент для диагностики сетевого соединения и устранения базовых проблем. Предлагает выбор между сбросом сетевых настроек и онлайн-проверкой доступности популярных ресурсов (ping, tracert). Подходит для использования без GUI.

**Применение:**  
- Сбросить сетевые настройки (IP, DNS, Winsock);
- Перезапустить сетевой адаптер;
- Запустить быструю диагностику соединения с популярными сайтами (пинг, трассировка, оценка стабильности);
- Проверить работоспособность службы Wi-Fi (Wlansvc);
- Использовать в корпоративной или домашней среде при жалобах на нестабильность соединения.


**Ключевые команды:**

- `ipconfig /all` — вывод полной конфигурации сетевых интерфейсов (IP, DNS, шлюз)
- `ipconfig /release`, `/renew`, `/flushdns` — Обновление IP-адреса и сброс DNS-кеша
...
- `sc config Wlansvc start= auto` / `net start Wlansvc` — Включение службы автонастройки WLAN
...
- Меню с выбором действий (reset / диагностика / выход);
- Проверка прав администратора;
- Сброс и обновление сетевых параметров;
- Диагностика подключения к vk.com, mail.ru, telegram, yandex;
- Отображение результатов ping, tracert, потерь при больших пакетах.
...
| `systeminfo >> "%LOG_DIR%\%DUMP_FILENAME%"`                               | Формирование дампа ОС в текстовом виде                                       |
...
#### Анализ системных логов (через `event_log_report.txt`)
...
Где `255.255.255.255` — IPv4 сервера.
...
* `reg add HKLM\SYSTEM\...\fDenyTSConnections /d 0` — разрешение входящих RDP-соединений через реестр;
* `netsh advfirewall firewall add rule name="Allow RDP"` — открытие порта 3389 в брандмауэре Windows;
* `sc config TermService start=auto` и `net start TermService` — автозапуск и запуск службы удалённого рабочего стола;
* `reg query HKLM\SYSTEM\...\fDenyTSConnections` — проверка текущего состояния RDP в реестре;
* `netstat -ano | findstr :3389` — проверка, прослушивается ли порт 3389 (службой TermService);
* `sc query TermService | findstr "STATE"` — получение текущего статуса службы RDP;
* `net user` — вывод списка всех локальных пользователей, среди которых нужно выбрать того, кто будет использовать RDP;
* `ipconfig /all` и `ipconfig | findstr "IPv4"` — определение текущего IP-адреса сервера для подключения.
...
| Синтаксис | Назначение / Пояснение |
|-----------|-------------------------|
| `net session >nul 2>&1` | Проверка запуска с правами администратора |
| `for /f "tokens=2 delims==." %%i in ('wmic os get localdatetime /value') do set now=%%i` | Получение текущей даты и времени через WMIC |
| `set logfile=%USERPROFILE%\Desktop\RDP_Server_Log_...` | Формирование уникального имени лог-файла с временной меткой |
| `setlocal enabledelayedexpansion` | Активация отложенного расширения переменных (важно для IF-блоков) |
| `color 0a`, `title ...`, `mode con: cols=80 lines=30` | Настройка внешнего вида окна консоли (цвет, заголовок, размер) |
| `echo [%time%] ... >> "%logfile%"` | Логирование этапов выполнения с временными отметками |
| `goto :label`, `:label` | Использование меток и переходов между блоками (в том числе для обработки ошибок) |
...
**Особенности:**
* Все действия логируются в файл `RDP_Server_Log_YYYYMMDD_HHMMSS.txt` на рабочем столе;
* Выполняется включение RDP, открытие порта в фаерволе, перезапуск служб, проверка учётных записей;
* Не требуется перезагрузка после выполнения;
* Скрипт не выполняет подключение, только подготовку сервера.

### `RDP_Client_Setup.bat` — диагностика и подключение клиента

**Ключевые команды:**

* `wmic os get localdatetime` — Генерация метки времени для лога
* `dism /online /Enable-Feature ...` — Включение RDP и Telnet клиента
* `netsh interface show interface` — Диагностика сетевых интерфейсов
* `ping -n 4 %target_ip%` — Проверка доступности IP
* `telnet %target_ip% 3389` — Проверка доступности порта 3389 (RDP)

**Синтаксис кода:**

| Синтаксис | Назначение / Пояснение |
|-----------|-------------------------|
| `for /f "tokens=2 delims==." %%i in ('wmic os get localdatetime /value') do set now=%%i` | Получение текущей даты и времени в формате `YYYYMMDD_HHMMSS` через WMIC |
| `set logfile=%USERPROFILE%\Desktop\RDP_Client_Log_...` | Создание лог-файла с меткой времени на рабочем столе |
| `setlocal enabledelayedexpansion` | Включение отложенного расширения переменных (нужно для корректной работы с переменными в блоках `if`) |
| `echo [%time%] ... >> "%logfile%"` | Логирование хода выполнения с точным временем |
| `set /p target_ip=...` | Получение IP-адреса от пользователя через интерактивный ввод |
| `if "%target_ip%"=="" (...)` | Проверка на пустой ввод (валидация IP) |
...
- Включение компонентов RDP и Telnet;
- Диагностика сетевой конфигурации;
- Запрос IP целевой машины;
- Проверка доступности по ping и порту 3389;
...
BAT-скрипт запускает установку `.msi` или `.exe`-установщика с параметром подробного логирования через встроенную утилиту `msiexec`. Это позволяет сохранить все этапы установки в лог-файл, который впоследствии можно проанализировать с помощью утилиты `WiLogUtl`. Удобен при отладке, обнаружении ошибок и передаче данных в техподдержку.

**Рекомендуется для использования:**
- При ошибках установки или обновления ПО;
- Для детального анализа действий установщика;
- При отладке кастомных MSI-пакетов;
- Перед обращением на L2/L3 уровень поддержки;
- В процессе ручного развёртывания ПО.
...
**WiLogUtl** — это графический инструмент, который визуализирует установочные логи `.msi` и облегчает поиск ошибок, условий сбоев, проблем доступа и несовпадений зависимостей.

## Установка WiLogUtl

`WiLogUtl` входит в состав **Windows SDK**.

1. Перейдите на официальный сайт: [sdk-archive](https://developer.microsoft.com/en-us/windows/downloads/sdk-archive)
